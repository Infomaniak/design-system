...
Added the `esds-kbd` custom attribute, which styles native `<kbd>` elements as keyboard keys (background, border, and typography driven by design tokens). The attribute can only be used on `<kbd>` elements, preserving native semantics. Multi-key shortcuts are composed by placing several `<kbd esds-kbd>` elements inline, separated by a `+` character.
