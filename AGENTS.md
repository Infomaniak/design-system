...
| Build wrappers only    | `yarn build:wrappers`                        |
...
- Monorepo with 3 workspaces:
  1. `@infomaniak-design-system/components` # Web components + CEM + framework wrappers
...
React, Angular and Vue wrappers are auto-generated from the CEM via `@pwrs/cem`:

| Subpath                                        | Format  | Build output            | Consumer requirement    |
| ---------------------------------------------- | ------- | ----------------------- | ----------------------- |
| `@infomaniak-design-system/components/react`   | ESM JS  | `dist/react/index.js`   | React >=19 (peer dep)   |
| `@infomaniak-design-system/components/angular` | TS src  | `dist/angular/index.ts` | Angular >=19 (peer dep) |
| `@infomaniak-design-system/components/vue`     | Vue SFC | `dist/vue/index.ts`     | Vue >=3 (peer dep)      |

Key rules:

- wrappers are generated by running `yarn build:wrappers` after `analyze:cem`
- `cem` binary collides between `@custom-elements-manifest/analyzer` and `@pwrs/cem`; always invoke both via their direct path in `node_modules`, never via `.bin` or `npx`/`yarn dlx`
- `build-wrappers.script.ts` post-processes generated wrappers to fix imports, strip `src/` paths, add `define()` calls, and remove readonly properties from wrapper props so framework consumers cannot set them
- React wrappers are bundled with Vite (`external: ['react', '@infomaniak-design-system/components']`)
- Angular and Vue wrappers are shipped as TypeScript/Vue SFC source; their respective build tools compile them as part of the consumer app
- `react`, `@angular/core` and `vue` are all **optional** `peerDependencies`; the library must build without them installed
