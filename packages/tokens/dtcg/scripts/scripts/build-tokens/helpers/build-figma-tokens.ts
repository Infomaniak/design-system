...
export async function buildFigmaTokens({
  ...
  const figmaTokens = mergeFigmaTokensAsModes(
    themeCollections.map((collection: DesignTokensCollection, index: number): [string, unknown] => {
      return [
  ...
  await writeFileSafe(`${outputDirectory}/figma.tokens.json`, JSON.stringify(figmaTokens, null, 2));
}
