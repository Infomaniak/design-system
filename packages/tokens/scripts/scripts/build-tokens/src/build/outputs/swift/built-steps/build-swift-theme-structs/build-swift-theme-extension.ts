...
function emitDiffNode(
    ...
    const args: string[] = [];
    const leafEntries = Object.entries(diffNode).filter(([, v]) => typeof v === 'string') as [string, string][];
    const subNodeEntries = Object.entries(diffNode).filter(([, v]) => typeof v !== 'string') as [string, DiffNode][];
    ...
    for (const [key, childDiff] of subNodeEntries) {
        ...
        if (!sharedName && childEntries.length === 1 && typeof childEntries[0][1] === 'string') {
            ...
            const call = emitDiffNode(childTypeName, childDiff, childTreeNode, childPath, patterns, modifierValueMap);
