...
export async function publishSvgComponentScript(): Promise<void> {
  return logger.asyncTask('publish-svg-component.script', async (logger: Logger): Promise<void> => {
...
try {
  await publishSvgComponentScript();
} catch (error: unknown) {
  logger.fatal(error);
}
